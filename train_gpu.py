...
def main():
    ...
    writer_ave = pd.ExcelWriter('{0}/training_ave_{1}.xlsx'.format(save_path, str_time), engine="openpyxl")
    # Training curve storage path (value of each validating instance)
    writer_100 = pd.ExcelWriter('{0}/training_100_{1}.xlsx'.format(save_path, str_time), engine="openpyxl")
    valid_results = []
    valid_results_100 = []
