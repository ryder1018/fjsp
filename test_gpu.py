...
import gym   # 可以保留，FJSPEnv 仍然繼承 gym.Env
import pandas as pd
import torch
import numpy as np
...
from env.fjsp_env import FJSPEnv   # ⭐ 直接使用環境類別本身
...
def main():
    ...
    writer = pd.ExcelWriter(
        "{0}/makespan_{1}.xlsx".format(save_path, str_time), engine="openpyxl"
    )  # Makespan data storage path
    writer_time = pd.ExcelWriter(
        "{0}/time_{1}.xlsx".format(save_path, str_time), engine="openpyxl"
    )  # time data storage path
    ...
    for i_rules in range(len(rules)):
        ...
        if rule.endswith(".pt"):
            if device.type == "cuda":
                ...
                model_CKPT = torch.load(
                    "./model/" + mod_files[i_rules], map_location="cpu"
                )
        ...
        for i_ins in range(num_ins):
            ...
            # Create environment object
                ...
                # Clear the existing environment if GPU mem too high
                meminfo = pynvml.nvmlDeviceGetMemoryInfo(handle)
                if meminfo.used / meminfo.total > 0.7:
                    envs.clear()

                # ⭐ 直接建立 FJSPEnv，不再經過 gym.make / wrapper
                if test_paras["sample"]:
                    # DRL-S, each env contains multiple copies of one instance
                    env = FJSPEnv(
                        case=[test_file] * test_paras["num_sample"],
        ...
        data.to_excel(
            writer, sheet_name="Sheet1", index=False, startcol=i_rules + 1
        )

        data = pd.DataFrame(torch.tensor(times).t().tolist(), columns=[rule])
        data.to_excel(
            writer_time, sheet_name="Sheet1", index=False, startcol=i_rules + 1
        )
