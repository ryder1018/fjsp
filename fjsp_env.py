...
class FJSPEnv(gym.Env):
    ...
    def next_time(self, flag_trans_2_next_time):
        ...
        # 還沒到現在時間的 available_time 保留，其餘設成一個很大的時間
        b = torch.where(
            a > self.time[:, None],
            a,
            torch.max(self.feat_opes_batch[:, 4, :]) + 1.0,
        )
        # 每個 batch 要前進到的時間 c
        c = torch.min(b, dim=1)[0]

        # 哪些機台在時間 c 完工，而且目前空閒，且該 batch 需要轉移
        d = (a == c[:, None]) & (self.machines_batch[:, :, 0] == 0) & flag_need_trans[:, None]

        # 每個 batch 的當前時間（要嘛前進到 c，要嘛維持原 time）
        e = torch.where(flag_need_trans, c, self.time)
        self.time = e

        # ---- 更新狀態與特徵 ----
        # 以前是 aa = self.machines_batch.transpose(1, 2); aa[d, 0] = 1; ...
        # 這在 PyTorch 2 會噴 IndexError，改成明確指定 (batch, machine) index
        batch_idxes, mas_idxes = torch.where(d)
        if batch_idxes.numel() > 0:
            self.machines_batch[batch_idxes, mas_idxes, 0] = 1.0  # 狀態設為完成
        ...
        # 找出在時間 c 完工的 job index
        jobs = torch.where(d, self.machines_batch[:, :, 3].double(), -1.0).float()
        # 原本是 np.argwhere(...).to(self.device)，在新 PyTorch / NumPy 也不太對，
        # 改成 torch.nonzero 版本
